...
module Validation
  ...
  def self.validate_name(player_name)
    ...
    puts e.message
    # если эта валидация срабатывает - то возвращаю true, это нужно чтобы срабатывал unless Validation.validate_name(player) в методе create_player
    true
  end
  ...
  def self.validate_hit(player)
    ...
    puts e.message
    # если эта валидация срабатывает - то возвращаю true, это нужно чтобы срабатывал unless Validation.validate_hit(player) в методе give_rcard
    true
  end
...
# Благодяря этому не нужно писать лишних обращений и локальных переменных (instance_variable.get set etc.) и городить в классах переменные класса,
# которые к тому же делают данные публичными 
# @@players = []
# @@game_counter = 0
# @@bank
...
module SharedVars
  ...
  module ClassMethods
    ...
    # !!! не проще ли создать класс game, там инициализовать эти переменные в конструкторе, а потом вызывать в нужных классах?
    attr_accessor :game_counter#, :all_players, :bank
    ...
    # def temp_show_all_players
    #   puts @@all_players.size
    #   p @@all_players
    # end
  ...
  module InstanceMethods
    ...
    #   ||= doesn't work for initializing class variables from class methods. We need to directly set the class variable.
    def increase_game_counter
      ...
      #  по умолчанию присваивется nil. Т.е nil - начальное значение и потому в случае nil я привожу значение game_counter к 0,

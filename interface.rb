...
# 3. закончить метод win_loose_check (done)
  ...
  # 3.2 Когда в player_selections игрок игрок выбирает hit - вставляем метод win_loose_check в котором проверяем: (done)
    # если busted (done)
    # если blackjack (done)
...
# 7. сделать валидации:
          ...
          # 9. Если сумма очков у игрока и дилера одинаковая, то объявляется ничья и деньги из банка возвращаются игрокам (done)
# 8. разбить на private - protected 
# 9. идиомы, DRY и rubocop
# 10. проблема! меню выбора hit|stand|reveal не отображается, если предыдущая игра закончилась (done!)
#     автоматическим вскрытием по достижении 3х карт у обоих игроков: Both players cards limit! Revealing! Перенес menu_text после reveal 
...
class Interface
  ...
  def initialize
    ...
    @winner_or_not = proc do
      ...
      winner.cash += Interface.get_bank
    end
  end
  ...
  def starting_game
    ...
    self.class.bets # self.class.bets - тоже что и Interface.bets. Т.к класс INterface расширен методами модуля SharedVars
    show_info
    # Interface.temp_show_all_players
  end
  ...
  def busted_case(player)
    not_busted_player = Interface.get_players.find { |player| !player.busted? }
    ...
    # giving bank to winner
    not_busted_player.cash += Interface.get_bank
  ...
  def bj_case(player)
    ...
    # giving bank to winner
    player.cash += Interface.get_bank
    # 1.clearing bank 2.show info 3.play again
    @empty_show_play.call
  end

  # win_loose_check helper #2
  # method compares score of both players. With max score becomes winner. If score same for both -no winner
  # cash
  ...
  def winner_no_winner#(player)
    # если кол-во очков не одинаковое у @p1 и @d1
    unless @p1.score == @d1.score # наверно для этого лучше сделать валидацию!!!
      @winner_or_not.call
  ...
  def create_player
    loop do
      ...
      unless Validation.validate_name(player_name) # !!! эта валидация должна быть в конструкторе класса player
      @p1 = Player.new(player_name); @d1 = Dealer.new
      # break if !Validation.validate_name(player_name) # according to code style -> favor unless over if for negative conditions
  ...
  def show_info(show_dealer_cards = false)
    ...
    puts "Game bank: #{self.class.get_bank}"
  end
  ...
  def play_again
    ...
    if (0..1).include?(select)
      # отличная идея со вложенным тернарником!
      ...
      select == 0 ? exit : (Interface.not_enough_cash? ? exit : go_again)
    else 
      puts 'Wrong input!'
      play_again
    end
